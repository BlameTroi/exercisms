...
func Test_mergeFreqMaps(t *testing.T) {
	...
	tests := []struct {
		...
		{"empty", []FreqMap{}, FreqMap{}},
		{"single", []FreqMap{{'a': 1, 'b': 1}}, FreqMap{'a': 1, 'b': 1}},
		{"double", []FreqMap{{'a': 1, 'b': 1}, {'a': 1, 'b': 1}}, FreqMap{'a': 2, 'b': 2}},
		{"mismatch", []FreqMap{{'a': 1}, {'b': 1}}, FreqMap{'a': 1, 'b': 1}},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := mergeFreqMaps(tt.args); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("mergeFreqMaps() = %v, want %v", got, tt.want)
			}
		})
	}
}
