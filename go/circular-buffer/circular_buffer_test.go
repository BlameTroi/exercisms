...
func Test_NewBuffer(t *testing.T) {
	b := NewBuffer(10)
	if !b.Empty() {
		t.Error("new buffer should be empty, is not")
	}
}

func Test_OneBuffer(t *testing.T) {
	b := NewBuffer(1)
	if !b.Empty() {
		t.Error("new buffer should be empty, is not")
	}
	...
	err := b.WriteByte('A')
	if err != nil {
		t.Errorf("unexpected error on write %q", err)
	}

	if b.Empty() {
		t.Error("buffer should not be empty, but it is")
	}

	c, err := b.ReadByte()
	if err != nil {
		t.Errorf("unexpected error on read %q", err)
	}
	if c != 'A' {
		t.Errorf("wrong value on read, got %x, want %x", c, 'A')
	}

	if !b.Empty() {
		t.Error("buffer should be empty, but it is not")
	}
}
...
func Test_WriteRead(t *testing.T) {
	...
	if c != 13 {
		t.Errorf("unexpected result from read, wanted 13 got %d", c)
	}
	c, err = b.ReadByte()
	if err != nil {
		t.Errorf("unexpected error on read %q", err)
	...
	_, err = b.ReadByte()
	if err == nil {
		t.Error("expected error on read, did not get")
	}
	...
	if c != 14 {
		t.Errorf("unexpected value on read, wanted 14 got %d", c)
	}
	b.Reset()
	_, err = b.ReadByte()
	if err == nil {
...
func Test_WrapAround(t *testing.T) {
	...
	assertNoError(t, err)
	if c != 'A' {
		t.Errorf("unexpected value on read, wanted %x got %x", 'A', c)
	}
	err = b.WriteByte('D')
	assertNoError(t, err)
	c, err = b.ReadByte()
	assertNoError(t, err)
	if c != 'B' {
		t.Errorf("unexpected value on read, wanted %x got %x", 'B', c)
	}
}
